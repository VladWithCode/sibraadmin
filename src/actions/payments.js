        ...
        fetch(url)
            ...
            .then(data => {
                ...
                const editorTemplates = data.templates.map(template => {
                    ...
                    const blocksContent = ContentState.createFromBlockArray(
                        ...blocks
                    );

                    console.log(draftToHtml(convertToRaw(EditorState.createWithContent(blocksContent).getCurrentContent())));
                    return {
                        ...template,
                        content: { editorState: EditorState.createWithContent(blocksContent) }
                    }
...
const isFormValid = paymentInfo => {
    const { liquidate, preReserved, depositAmount, reservationDate, preReservationAmount, preReservationDate, history, payments, lapseToPay, recordOpenedAt, paymentsDate, lapseType } = paymentInfo;
...
export const submitPayment = (paymentInfo, lotInfo) => {
    ...
    return (dispatch) => {
        ...
        if (isValid) {
            ...
            const { liquidate, preReserved, client, depositAmount, payedTo, payedAt, amount, reservationDate, preReservationAmount, preReservationDate, history, payments, lapseToPay, recordOpenedAt, paymentsDate, lapseType } = paymentInfo;
            const { projectId, lotId, lotPrice } = lotInfo;

            const state = liquidate ? 'liquidated' : preReserved ? 'preReserved' : "reserved"
