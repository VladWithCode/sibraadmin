...
export const PaymentsEdit = ({ recordId }) => {
  ...
  const hasChanged = (payment) => {
    const compareTo = currentRecord?.payments.find(
      (p) => p._id === payment._id
    );
    ...
    if (
      compareTo?.payedAt.split("T")[0] === payment.payedAt.split("T")[0] ||
      compareTo.payedAt.split("T")[0] === payment.payedAt
    ) {
      return false;
    }
