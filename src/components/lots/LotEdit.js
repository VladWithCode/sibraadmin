...
export const LotEdit = () => {
    ...
    useEffect(() => {
        ...
        dispatch(redirectSet(redTypes.projects, `/proyectos/edit/${projectId}/lote/${lotId}`));
    ...
    return (
        <div className="pb-5 project create">
            ...
            <ModalDoc fileName={fileInfo.fileName} type={redTypes.lot} id={lotId} projectId={projectId} />
            ...
            <div className="card edit">
                ...
                <div className="card__body">
                    ...
                    <div className="right">
                        ...
                        <div className={`card__body__item ${emptyFields.includes('area') && 'error'} mt-5`}>
                            <label htmlFor="area">Área</label>
                            <input onChange={handleChange} className={`${hasChanged.includes('area') && 'changed'}`} name="area" type="number" autoComplete="off" value={area} />
                        </div>
                        <div className={`card__body__item ${emptyFields.includes('price') && 'error'}`}>
                            <label htmlFor="price">Precio</label>
                            <input onChange={handleChange} className={`${hasChanged.includes('price') && 'changed'}`} autoFocus name="price" type="number" autoComplete="off" value={price} />
                        </div>
                    </div>
                    <div className="left">
                        <div className="card__header">
                            ...
                            <button onClick={addRef} className="add-ref">Agregar medida</button>
                            ...
                            measures.length > 0 && (
                                ...
                                measures.map((measure, index) => (
                                    <div key={`measure${index}`}>
                                        <div className={`card__body__item ${emptyFields.includes(`measure-title${index}`) && 'error'} mt-3`}>
                                            <button onClick={(e) => deleteMeasure(index)} className="delete-area measure">
                                                &times;
                                            </button>
                                            <label htmlFor={`measure-title${index}`}>Nombre de medida</label>
                                            <input onChange={e => handleChangeMeasure(index, e, `title`)} className={`${hasChanged.includes(`measure-title${index}`) && 'changed'}`} name={`measure-title${index}`} type="text" autoComplete="off" value={measure.title} />
                                        </div>
                                        <div key={measure._id} className={`card__body__item ${emptyFields.includes(`measure-value${index}`) && 'error'}`}>
                                            <label htmlFor={`measure-value${index}`}>Medida en m<sup>2</sup> </label>
                                            <input onChange={e => handleChangeMeasure(index, e, 'value')} className={`${hasChanged.includes(`measure-value${index}`) && 'changed'}`} name={`measure-value${index}`} type="text" autoComplete="off" value={measure.value} />
                                        </div>
                                    </div>
                                )
                                )
                        ...
                        <div className="full">
                            ...
                            <div className="file-form mt-2">
                                <div className={`card__body__item`}>
                                    ...
                                    <input type="text" name="fileName" onChange={handleInputFileChange} value={fileName} />
                                </div>
