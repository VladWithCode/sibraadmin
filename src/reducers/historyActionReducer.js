...
export const historyActionsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case redTypes.historySetPayment:
      ...
      const index = state.record.payments.findIndex(
        (p) => p._id === payment._id
      );
    ...
    case redTypes.historySetExtraPayment:
      ...
      const extraCharge = state.record.extraCharges.find(
        (e) => e._id === action.payload.extraChargeId
      );
      ...
      extraPayment.valid =
        extraPayment.date !== "" && extraPayment.amount !== "";

      const tempInvalidExtraPayments = state.record.invalidExtraPayments.find(
        (i) => i.extraChargeId === extraCharge._id
      );

      if (!extraPayment.valid && !extraPayment.delete) {
        if (tempInvalidExtraPayments) {
          if (
            !tempInvalidExtraPayments.invalidPayments.find(
              (inp) => inp._id === extraPayment._id
            )
          ) {
        ...
        (extraPayment.valid && !extraPayment.delete) ||
        extraPayment.delete
      ) {
        if (tempInvalidExtraPayments) {
          let index = tempInvalidExtraPayments.invalidPayments.indexOf(
            extraPayment._id
          );
          ...
          if (index !== -1) {
            ...
            let tempIndex = state.record.invalidExtraPayments.findIndex(
              (iep) =>
                iep.extraChargeId === tempInvalidExtraPayments.extraChargeId
            );
      ...
      const extraPaymentIndex = extraCharge.payments.findIndex(
        (p) => p._id === extraPayment._id
      );
    ...
    case redTypes.historySetExtraPayBefore:
      const tempExtraCharge = state.record.extraCharges.find(
        (e) => e._id === action.payload.extraChargeId
      );
